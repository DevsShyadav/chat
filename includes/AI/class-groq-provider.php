...
class Groq_Provider implements Provider_Interface {
    ...
    public function chat( $messages, $system_prompt = '', $options = array() ) {
        if ( ! $this->is_configured() ) {
            return array(
                'success'     => false,
                'error'       => __( 'Groq API key is not configured.', 'wp-ai-chatbot' ),
                'content'     => '',
                'tokens_used' => 0,
            );
        }
        ...
        if ( 200 !== $status_code ) {
            $error_message = isset( $response_body['error']['message'] ) ? $response_body['error']['message'] : __( 'Unknown Groq API error', 'wp-ai-chatbot' );
            return array(
                'success'     => false,
                'error'       => $error_message,
                'content'     => '',
